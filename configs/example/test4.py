...
for i, cpu in enumerate(cpus):
    # Directly access the committedInsts stat from the CPU object
    insts = cpu.committedInsts.value if hasattr(cpu, "committedInsts") else 0
    print(f"CPU{i} committed instructions: {insts}")
    ipc = insts / total_cycles if total_cycles else 0
    print(f"CPU{i} IPC = {ipc:.3f}")
